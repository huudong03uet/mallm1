...
class Scheduler:
    ...
    def __init__(self, config: Config) -> None:
        ...
        try:
            ...
            with open(config.input_json_file_path, encoding="utf-8") as f:
    ...
    def run_discussion(
        ...
        try:
            with open(self.config.output_json_file_path, "w", encoding="utf-8") as file:
                file.write(
                    json.dumps(self.output_dicts, ensure_ascii=False)
    ...
    def run_ablation(
        ...
        try:
            ...
            with open(
                out_path.with_name(out_path.stem + "-ablation.json", encoding="utf-8"), "w"
            ) as file:
                file.write(
                    json.dumps(self.ablation_output_dicts, ensure_ascii=False)
                )  # TODO: ensure correct json formatting (sometimes there is an invalid escape sequence warning)
                file.truncate()
        except Exception as e:
